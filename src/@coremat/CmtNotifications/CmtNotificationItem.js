...
const CmtNotificationItem = ({
  ...
  return (
    <Box
      className={clsx(classes.root, "CmtNotificationItem-root", className)}
      {...rest}
    >
      ...
      <Box display="flex" flexDirection="column" alignItems="center" ml={3}>
        ...
        <Tooltip title={readState ? "Mark as unread" : "Mark as read"}>
          <IconButton
            className={clsx(classes.dotBtn, "Cmt-dot-btn")}
            size="small"
            onClick={onReadUnread}
          >
            {readState ? (
              readIcon || <Brightness1Icon />
            ) : (
              unReadIcon || <RadioButtonUncheckedIcon />
            )}
          </IconButton>
        </Tooltip>
      </Box>
