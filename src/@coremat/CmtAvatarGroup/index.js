  ...
  rootAvatarGroup: props => ({
    "& .MuiAvatar-root": {
      ...
      "&:hover": {
        ...
        boxShadow: `rgba(0,0,0,0.2) 0px 2px ${props.avatarActiveStyle
          .elevation}px -1px`,
...
const CmtAvatarGroup = ({
  ...
  const restItems =
    max < totalRecords ? items.slice(max - 1, totalRecords) : [];

  const [ openMoreTooltip, setOpenMoreTooltip ] = useState(false);
  const [ renderedMoreItems, setRenderedMoreItems ] = useState(
    moreVisibleOn && moreVisibleOn !== "click" && restItems.length && renderMore
  ...
  return (
    <AvatarGroup
      className={clsx(classes.rootAvatarGroup, "Cmt-root-avatar-group")}
      max={max}
      {...rest}
    >
